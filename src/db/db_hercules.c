...
int item_package_he_load(const char * path, native_t * native) {
    int i = 0;
    int len = 0;
    config_t * config = NULL;
    config_setting_t * root = NULL;
    item_package_he * item_packages = NULL;
    ...
    if (0 >= len) {
        fprintf(stderr, "[load]: \"item_package\" is empty.\n");
        goto failed;
    }

    item_packages = calloc(len, sizeof(item_package_he));
    if (NULL == item_packages)
        goto failed;

    for (i = 0; i < len; i++)
        if (item_package_he_load_record(config_setting_get_elem(root, i), &item_packages[i]))
            goto failed;

    native->db = item_packages;
    native->size = len;
...
failed:
    SAFE_FREE(item_packages);
    config_destroy(config);
    SAFE_FREE(config);
    return CHECK_FAILED;
}

int item_package_he_load_record(config_setting_t * record, item_package_he * item_package) {
    int i = 0;
    int len = 0;
    char * name = NULL;
    ...
    item_package->name = convert_string(name);
    if (NULL == item_package->name)
        return CHECK_FAILED;
    ...
    item_package->items = calloc(len, sizeof(item_package_record_he));
    if (NULL == item_package->items)
        goto failed;

    for (i = 0; i < len; i++)
        if (item_package_he_load_record_item(config_setting_get_elem(record, i), &item_package->items[i]))
            goto failed;

    item_package->size = len;
    return CHECK_PASSED;

failed:
    SAFE_FREE(item_package->items);
    SAFE_FREE(item_package->name);
    return CHECK_FAILED;
}

int item_package_he_load_record_item(config_setting_t * record, item_package_record_he * item) {
    char * name = NULL;
    ...
    if(CONFIG_TRUE != config_setting_lookup_int(record, "Random", &item->random))
        item->random = 0;
    if(CONFIG_TRUE != config_setting_lookup_int(record, "Count", &item->count))
        item->count = 0;
    if(CONFIG_TRUE != config_setting_lookup_int(record, "Expire", &item->expire))
        item->expire = 0;
    if(CONFIG_TRUE != config_setting_lookup_bool(record, "Announce", &item->announce))
        item->announce = 0;
    if(CONFIG_TRUE != config_setting_lookup_int(record, "Rate", &item->rate))
        item->rate = 0;
    if(CONFIG_TRUE != config_setting_lookup_bool(record, "Named", &item->named))
        item->named = 0;

    return CHECK_PASSED;
}

int item_package_he_free(item_package_he * item_packages, int size) {
    int i = 0;
    int j = 0;
    if (NULL == item_packages || 0 >= size)
        return CHECK_FAILED;
    for (i = 0; i < size; i++) {
        for (j = 0; j < item_packages[i].size; j++)
            SAFE_FREE(item_packages[i].items[j].name);
        SAFE_FREE(item_packages[i].items);
        SAFE_FREE(item_packages[i].name);
    }
    SAFE_FREE(item_packages);
    return CHECK_PASSED;
}
...
failed:
    item_chain_he_free(item_chains, len);
    config_destroy(config);
    SAFE_FREE(config);
    return CHECK_FAILED;
}
...
failed:
    item_group_he_free(item_groups, len);
    config_destroy(config);
    SAFE_FREE(config);
    return CHECK_FAILED;
}
...
int herc_db_init(herc_db_t ** db, const char * path) {
    ...
    if( SQLITE_OK != sqlite3_prepare_v2(herc->db, HERC_ITEM_INSERT, (int) strlen(HERC_ITEM_INSERT), &herc->item_he_sql_insert, NULL)          ||
        SQLITE_OK != sqlite3_prepare_v2(herc->db, HERC_MOB_INSERT, (int) strlen(HERC_MOB_INSERT), &herc->mob_he_sql_insert, NULL)             ||
        SQLITE_OK != sqlite3_prepare_v2(herc->db, HERC_SKILL_INSERT, (int) strlen(HERC_SKILL_INSERT), &herc->skill_he_sql_insert, NULL)       ||
        SQLITE_OK != sqlite3_prepare_v2(herc->db, HERC_PRODUCE_INSERT, (int) strlen(HERC_PRODUCE_INSERT), &herc->produce_he_sql_insert, NULL) ||
        SQLITE_OK != sqlite3_prepare_v2(herc->db, HERC_MERC_INSERT, (int) strlen(HERC_MERC_INSERT), &herc->mercenary_he_sql_insert, NULL)     ||
        SQLITE_OK != sqlite3_prepare_v2(herc->db, HERC_PET_INSERT, (int) strlen(HERC_PET_INSERT), &herc->pet_he_sql_insert, NULL)             ||
        SQLITE_OK != sqlite3_prepare_v2(herc->db, HERC_CONST_INSERT, (int) strlen(HERC_CONST_INSERT), &herc->const_he_sql_insert, NULL)       ||
        SQLITE_OK != sqlite3_prepare_v2(herc->db, HERC_COMBO_INSERT, (int) strlen(HERC_COMBO_INSERT), &herc->item_combo_he_sql_insert, NULL)  ||
        SQLITE_OK != sqlite3_prepare_v2(herc->db, HERC_ITEM_SEARCH, (int) strlen(HERC_ITEM_SEARCH), &herc->item_he_sql_search, NULL)          ||
        SQLITE_OK != sqlite3_prepare_v2(herc->db, ITEM_GROUP_INSERT, (int) strlen(ITEM_GROUP_INSERT), &herc->item_group_he_insert, NULL)      ||
        SQLITE_OK != sqlite3_prepare_v2(herc->db, ITEM_GROUP_RECORD_INSERT, (int) strlen(ITEM_GROUP_RECORD_INSERT), &herc->item_group_record_he_insert, NULL) ||
        SQLITE_OK != sqlite3_prepare_v2(herc->db, ITEM_CHAIN_INSERT, (int) strlen(ITEM_CHAIN_INSERT), &herc->item_chain_he_insert, NULL)      ||
        SQLITE_OK != sqlite3_prepare_v2(herc->db, ITEM_CHAIN_RECORD_INSERT, (int) strlen(ITEM_CHAIN_RECORD_INSERT), &herc->item_chain_record_he_insert, NULL) ||
        SQLITE_OK != sqlite3_prepare_v2(herc->db, ITEM_PACKAGE_INSERT, (int) strlen(ITEM_PACKAGE_INSERT), &herc->item_package_he_insert, NULL) ||
        SQLITE_OK != sqlite3_prepare_v2(herc->db, ITEM_PACKAGE_RECORD_INSERT, (int) strlen(ITEM_PACKAGE_RECORD_INSERT), &herc->item_package_record_he_insert, NULL)) {
...
int herc_db_deit(herc_db_t ** db) {
    ...
    if( SQLITE_OK != sqlite3_finalize(herc->item_he_sql_insert) ||
        ...
        SQLITE_OK != sqlite3_finalize(herc->item_package_he_insert) ||
        SQLITE_OK != sqlite3_finalize(herc->item_package_record_he_insert) ||
...
int herc_load_item_db_insert(item_he * db, int size, sqlite3_stmt * sql) {
    ...
    for(i = 0; i < size; i++) {
        ...
        if( SQLITE_OK != sqlite3_clear_bindings(sql) ||
            ...
            SQLITE_OK != sqlite3_bind_text(sql, 3, item->name, (int) strlen(item->name), SQLITE_STATIC) ||
            ...
            SQLITE_OK != sqlite3_bind_text(sql, 40, item->script, (int) strlen(item->script), SQLITE_STATIC) ||
            SQLITE_OK != sqlite3_bind_text(sql, 41, item->onequipscript, (int) strlen(item->onequipscript), SQLITE_STATIC) ||
            SQLITE_OK != sqlite3_bind_text(sql, 42, item->onunequipscript, (int) strlen(item->onunequipscript), SQLITE_STATIC) ||
        ...
        if(item_id_array != NULL && item_id_size > 0) {
            ...
            for(j = 0; j < item_id_size; j++) {
                if( SQLITE_OK != sqlite3_clear_bindings(sql) ||
                    ...
                    SQLITE_OK != sqlite3_bind_text(sql, 2, combo->script, (int) strlen(combo->script), SQLITE_STATIC) ||
...
int herc_load_item_group_db_insert(herc_db_t * db, item_group_he * item_groups, int size, sqlite3_stmt * sql) {
    ...
    for (i = 0; i < size; i++) {
        item_group = &item_groups[i];
        if (SQLITE_OK != sqlite3_clear_bindings(sql) ||
            SQLITE_OK != sqlite3_bind_text(sql, 1, item_group->name, (int) strlen(item_group->name), SQLITE_STATIC) ||
            SQLITE_DONE != sqlite3_step(sql) ||
            SQLITE_OK != sqlite3_reset(sql)) {
...
int herc_load_item_group_record_db_insert(item_group_record_he * items, int size, sqlite3_stmt * sql, int group_id) {
    ...
    for (i = 0; i < size; i++) {
        item = &items[i];
        if (SQLITE_OK != sqlite3_clear_bindings(sql) ||
            SQLITE_OK != sqlite3_bind_text(sql, 1, item->name, (int) strlen(item->name), SQLITE_STATIC) ||
...
int herc_load_item_chain_db_insert(herc_db_t * db, item_chain_he * item_chains, int size, sqlite3_stmt * sql) {
    ...
    for (i = 0; i < size; i++) {
        ...
        if (SQLITE_OK != sqlite3_clear_bindings(sql) ||
            SQLITE_OK != sqlite3_bind_text(sql, 1, item_chain->name, (int) strlen(item_chain->name), SQLITE_STATIC) ||
            SQLITE_DONE != sqlite3_step(sql) ||
            SQLITE_OK != sqlite3_reset(sql)) {
            fprintf(stderr, "[load]: failed to add %s to item group db.\n", item_chain->name);
            if (SQLITE_OK != sqlite3_reset(sql)) {
...
int herc_load_item_package_db(herc_db_t * db, const char * path) {
    native_t item_packages;
    item_packages.db = NULL;
    item_packages.size = 0;

    if (item_package_he_load(path, &item_packages) ||
    ...
    if (herc_db_exec(db, "BEGIN IMMEDIATE TRANSACTION;") ||
        herc_load_item_package_db_insert(db, item_packages.db, item_packages.size, db->item_package_he_insert) ||
        herc_db_exec(db, "COMMIT TRANSACTION;")) {
        fprintf(stderr, "Failed to load item package db.\n");
        goto failed;
    }
    fprintf(stdout, "[load]: loaded item package db; %s\n", path);

    item_package_he_free(item_packages.db, item_packages.size);
    return CHECK_PASSED;
...
int herc_load_item_package_db_insert(herc_db_t * db, item_package_he * item_packages, int size, sqlite3_stmt * sql) {
    int i = 0;
    int id = 0;
    item_package_he * item_package = NULL;

    for (i = 0; i < size; i++) {
        item_package = &item_packages[i];
        if (SQLITE_OK != sqlite3_clear_bindings(sql) ||
            SQLITE_OK != sqlite3_bind_text(sql, 1, item_package->name, (int) strlen(item_package->name), SQLITE_STATIC) ||
            SQLITE_DONE != sqlite3_step(sql) ||
            SQLITE_OK != sqlite3_reset(sql)) {
...
int herc_load_item_package_record_db_insert(item_package_record_he * items, int size, sqlite3_stmt * sql, int package_id) {
    int i = 0;
    item_package_record_he * item = NULL;

    for (i = 0; i < size; i++) {
        item = &items[i];
        if (SQLITE_OK != sqlite3_clear_bindings(sql) ||
            SQLITE_OK != sqlite3_bind_text(sql, 1, item->name, (int) strlen(item->name), SQLITE_STATIC) ||
            SQLITE_OK != sqlite3_bind_int(sql, 2, item->random) ||
            SQLITE_OK != sqlite3_bind_int(sql, 3, item->count) ||
            SQLITE_OK != sqlite3_bind_int(sql, 4, item->expire) ||
            SQLITE_OK != sqlite3_bind_int(sql, 5, item->announce) ||
            SQLITE_OK != sqlite3_bind_int(sql, 6, item->rate) ||
            SQLITE_OK != sqlite3_bind_int(sql, 7, item->named) ||
            SQLITE_DONE != sqlite3_step(sql) ||
            SQLITE_OK != sqlite3_reset(sql)) {
