...
int main(int argc, char * argv[]) {
    ...
    while(lua_next(lstate, 1)) {
        ...
        if(ncs_strcmp(server_type,"eathena") == 0)
            ...
            script.mode = HERCULE;
        if(init_db_load(&script.db, resource_db_path, server_db_path, script.mode)) {
            fprintf(stderr, "[fail]: failed to load resource and server databases.\n");
            break;
        }
